...
\subsection{Hyperledger Fabric structure}
%The first characteristic of Fabric is that it is private and permissioned. The participants of a Fabric network, indeed, have to enroll via an MSP (Membership Service Provider). This guarantees a complete control of the network and the possibility of identifying different types of members. Before describing these different roles, however, we need to introduce a component that has not been mentioned yet, smart contracts.
...
Smart contracts are another concept related to a blockchain network: in general, they are self-executing actions that are triggered by some conditions. For example Ethereum, another cryptocurrency based on a permissionless blockchain, allows developers to program their ``autonomous agents'', using a particular DSL (Domain Specific Language), Solidity. A smart contract can be used, for instance, so that funds are spent only when all the members of an organization (or a percentage) give their authorization. 
...
In our use case, smart contracts are used to manipulate the state of the goods.

\subsubsection{Ledger}


\subsubsection{Peers}
Being one of the main reasons for which we chose this framework, the first characteristic of Fabric is that it is private and permissioned. The participants of a Fabric network, indeed, have to enroll via an MSP (Membership Service Provider). This guarantees a complete control of the network and the possibility of identifying different types of members. 
...
\subsection{Hyperledger Composer and Explorer}
How we used them to build the network
